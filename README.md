...
`assets/logo.svg` 是共用向量標誌，同時用於中英文導覽列及分頁圖示。
T／L 取自 Ting-Fu Lai；24 個刻度與圓環呼應全天活動行為及晝夜節律；
陶土色弧段與圓點強調「時間點」這項研究主軸。圖形為品牌意象，並非研究數據或建議活動時段。
標誌以 SVG 路徑繪製，不依賴字型，放大不會失真。


## 閱讀字級

右下角「字體大小」可切換標準、大、特大，內文預設約 18px，另有 20px、22px（以瀏覽器預設 16px 計算）。
閱讀文字採 rem 單位；偏好存於訪客自己的 localStorage，中英文共用。
若瀏覽器不允許儲存，當次仍可切換；停用 JavaScript 時仍使用放大後的預設字級。
